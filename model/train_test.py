...
with tf.Session() as session:
  ...
  #
  if not os.path.isfile('bin/models_seed='+str(seed)+'/last.ckpt.index'):
  ...
  #
    ...
    saver.restore(session, 'bin/models_seed='+str(seed)+'/last.ckpt')
  ...
  for iteration in range(num_iterations):
    ...
    if i_total%500 == 0 and i_total > 0:
      saver.save(session, 'bin/models_seed='+str(seed)+'/iteration='+str(i_total)+'.ckpt')

  # Save model
  #
  saver.save(session, 'bin/models_seed='+str(seed)+'/last.ckpt')
